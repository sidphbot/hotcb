...
class TestWeightsMutation:
    def test_distill_w_suffix(self):
        ...
        ls = _make_loss_state()
        result = ctrl.apply_op(_op(params={"distill_w": 0.2, "depth_w": 1.5}), {"loss_state": ls})
    ...
    def test_fallback_to_weights_bucket(self):
        ctrl = HotLossController()
        ls = _make_loss_state()
        result = ctrl.apply_op(_op(params={"custom_metric": 0.5}), {"loss_state": ls})
        assert result.decision == "applied"
...
class TestTermsToggle:
    def test_terms_dot_prefix(self):
        ctrl = HotLossController()
        ls = _make_loss_state()
        result = ctrl.apply_op(_op(params={"terms.aux_depth": False, "terms.aux_heatmap": True}), {"loss_state": ls})
        assert result.decision == "applied"
    ...
    def test_terms_as_dict(self):
        ctrl = HotLossController()
        ls = _make_loss_state()
        result = ctrl.apply_op(_op(params={"terms": {"aux_depth": False, "aux_heatmap": True}}), {"loss_state": ls})
        assert result.decision == "applied"
        assert ls["terms"]["aux_depth"] is False
        assert ls["terms"]["aux_heatmap"] is True
...
class TestRamps:
    def test_ramps_dot_prefix(self):
        ...
        result = ctrl.apply_op(_op(params={"ramps.depth": ramp_cfg}), {"loss_state": ls})
        assert result.decision == "applied"
...
class TestMissingLossState:
    def test_no_loss_state_in_env(self):
        ctrl = HotLossController()
        result = ctrl.apply_op(_op(params={"distill_w": 0.2}), {})
        assert result.decision == "failed"
        assert result.error == "missing_loss_state"

    def test_resolve_loss_state_callable(self):
        ctrl = HotLossController()
        ls = _make_loss_state()
        env = {"resolve_loss_state": lambda: ls}
...
class TestStatus:
    def test_status_structure(self):
        ctrl = HotLossController()
        ls = _make_loss_state()
        ctrl.apply_op(_op(params={"distill_w": 0.2}), {"loss_state": ls})
        status = ctrl.status()
...
class TestUnknownOp:
    def test_unknown_op_ignored(self):
        ctrl = HotLossController()
        result = ctrl.apply_op(HotOp(module="loss", op="reset", id="main"), {"loss_state": {}})
