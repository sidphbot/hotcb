...
Parameters ending in `_w` map to `loss_state["weights"]` with the suffix stripped:

```bash
hotcb loss set_params distill_w=0.2 depth_w=1.5
# -> loss_state["weights"]["distill"] = 0.2
# -> loss_state["weights"]["depth"] = 1.5
```
...
# -> loss_state["terms"]["aux_depth"] = False
# -> loss_state["terms"]["aux_heatmap"] = True
```
...
# -> loss_state["ramps"]["depth"] = {"type": "linear", "warmup_frac": 0.2, "end": 2.0}
```

### Fallback

Any parameter that does not match the above patterns is placed directly in `loss_state["weights"]`.

## Expected `loss_state` Shape
...
1. `env["loss_state"]` -- direct mutable dict reference.
2. `env["resolve_loss_state"]` -- callable that returns the dict.

If neither is available, the op fails with `missing_loss_state`.

Framework adapters accept a `loss_state` dict at construction time and inject it into every `env`.
